...
export class BesitosOfferwall {
  ...
  async buildUrl(): Promise<string> {
    ...
    if (config.deviceId?.trim())
      params.set("device_id", config.deviceId.trim());
    ...
    return `${SDK_CONFIG.BASE_URL}/${encodeURIComponent(config.partnerId)}/offers?${params.toString()}`;
  }

  static validate(config: OfferwallConfig): {
    valid: boolean;
    reason?: string;
  } {
